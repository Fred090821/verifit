...
/**
 * This class test that 7 new members will have 7 new entries in the streak record
 * but one new member with 7 attendance on the same week or day will only have 1 entry in the streak record
 * as it is in the same week.
 */
@DisplayName("StreakMetricsRepositoryTest.")
class StreakMetricsRepositoryTest extends BaseRepositoryTest {
  ...
  void whenNewMembersAreCreatedShouldHaveSevenRelatedStreakMetrics() {
    ...
    assertEquals(true, streakMetricsRecordEntity.stream().anyMatch(x -> x.getMemberId() == MEMBER_ID_TO_BE_TESTED_FOR_CREATED));
  ...
  @Test
  void noMatterHowManyTimeSameDaySameMemberAttendStreakMetricsIsSameForSameDay() {
    ...
    assertEquals(7, attendanceRecordEntities.size());
    assertEquals(true, attendanceRecordEntities.stream().allMatch(x -> x.getMemberId() == MEMBER_ID_TO_BE_TESTED_FOR_CREATED));
    assertEquals(true, attendanceRecordEntities.stream().allMatch(x -> x.getAttendanceDate().equals(TODAY_ATTENDANCE_DATE)));
    ...
    assertEquals(true, streakMetricsRecordEntity.get(0).getMemberId() == MEMBER_ID_TO_BE_TESTED_FOR_CREATED);
